...
export const DEFAULT_WORKOUT_DAYS = [1, 3, 5]; // Mon / Wed / Fri

// Schedule with an optional ramp-up (prep) phase before the main 28-week plan.
// opts.workoutDays = array of weekday indexes (0=Sun…6=Sat) the user trains.
export function getSchedule(startDate, date, daysBetween, opts = {}) {
  const prepWeeks = opts.prepWeeks || 0;
  const days = (Array.isArray(opts.workoutDays) && opts.workoutDays.length)
    ? [...new Set(opts.workoutDays)].sort((a, b) => a - b)
  ...
  const deload = mainWeek >= 8 && mainWeek % 4 === 0; // planned recovery
  let workoutKey = null;
  if (idx >= 0) {
    if (phase === 1) {
      workoutKey = "fullbody";
    } else {
      // upper/lower split spread across the chosen days, rotating weekly so all
      // four sessions get hit even if fewer than 4 days are selected.
      const seq = ["upperA", "lowerA", "upperB", "lowerB"];
      const off = (mainWeek - 1) % seq.length;
      workoutKey = seq[(idx + off) % seq.length];
    }
  }
  return { dss, beforeStart: false, prep: false, phase, week: mainWeek, workoutKey, rest: !workoutKey, deload, weekLabel: `Week ${mainWeek}/28${deload ? " · Deload" : ""}` };
}
